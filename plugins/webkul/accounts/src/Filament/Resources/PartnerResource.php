...
class PartnerResource extends BasePartnerResource
{
    ...
    public static function form(Schema $schema): Schema
    {
            ...
            Group::make()
                ->schema([
                    Select::make('user_id')
                        ...
                        ->preload()
                        ->searchable()
                        ->label(__('accounts::filament/resources/partner.form.fields.sales-person')),
                    Select::make('property_payment_term_id')
                        ->relationship('propertyPaymentTerm', 'name')
                        ->preload()
                        ->searchable()
                        ->label(__('accounts::filament/resources/partner.form.fields.payment-terms')),
                    Select::make('property_inbound_payment_method_line_id')
                        ->relationship('propertyInboundPaymentMethodLine', 'name')
                        ->preload()
                        ->searchable()
                        ->label(__('accounts::filament/resources/partner.form.fields.payment-method')),
                ])
                ->columns(2),
        ]);

        $purchaseComponents = Fieldset::make(__('accounts::filament/resources/partner.form.fields.purchase'))
            ->schema([
                Group::make()
                    ->schema([
        ...
        $fiscalInformation = Fieldset::make(__('accounts::filament/resources/partner.form.fields.fiscal-information'))
            ->schema([
                Group::make()
                    ->schema([
                        Select::make('property_account_position_id')
                            ->label(__('accounts::filament/resources/partner.form.fields.fiscal-position'))
        ...
        $invoicingComponent = Tab::make(__('accounts::filament/resources/partner.form.tabs.invoicing.title'))
            ...
            ->schema([
                Fieldset::make(__('accounts::filament/resources/partner.form.tabs.invoicing.fields.customer-invoices'))
                    ->schema([
                        Select::make('invoice_sending_method')
                            ->label('Invoice Sending Method')
                            ->label(__('accounts::filament/resources/partner.form.tabs.invoicing.fields.invoice-sending-method'))
                            ->options(InvoiceSendingMethod::class),
                        Select::make('invoice_edi_format_store')
                            ->label(__('accounts::filament/resources/partner.form.tabs.invoicing.fields.invoice-edi-format-store'))
                            ->live()
                            ->options(InvoiceFormat::class),
                        Group::make()
                            ->schema([
                                Select::make('peppol_eas')
                                    ->label(__('accounts::filament/resources/partner.form.tabs.invoicing.fields.peppol-eas'))
                                    ->live()
                                ...
                                TextInput::make('peppol_endpoint')
                                    ->label(__('accounts::filament/resources/partner.form.tabs.invoicing.fields.endpoint'))
                                    ->live()
                                    ->visible(fn (Get $get) => $get('invoice_edi_format_store') !== InvoiceFormat::FACTURX_X_CII->value && ! empty($get('invoice_edi_format_store'))),
                            ])->columns(2),
                    ]),

                Fieldset::make(__('accounts::filament/resources/partner.form.tabs.invoicing.fields.automation'))
                    ->schema([
                        Select::make('autopost_bills')
                            ->label(__('accounts::filament/resources/partner.form.tabs.invoicing.fields.auto-post-bills'))
                            ->options(AutoPostBills::class),
                        Toggle::make('ignore_abnormal_invoice_amount')
                            ->inline(false)
                            ->label(__('accounts::filament/resources/partner.form.tabs.invoicing.fields.ignore-abnormal-invoice-amount')),
                        Toggle::make('ignore_abnormal_invoice_date')
                            ->inline(false)
                            ->label('Ignore abnormal invoice date')
                            ->label(__('accounts::filament/resources/partner.form.tabs.invoicing.fields.ignore-abnormal-invoice-date')),
                    ]),
            ]);
    ...
    public static function infolist(Schema $schema): Schema
    {
            ...
            Group::make()
                ->schema([
                    TextEntry::make('user.name')
                        ->placeholder('-')
                        ->label(__('accounts::filament/resources/partner.infolist.entries.sales-person'))
                        ->icon('heroicon-o-user'),
                    TextEntry::make('propertyPaymentTerm.name')
                        ->placeholder('-')
                        ->label(__('accounts::filament/resources/partner.infolist.entries.payment-terms'))
                        ->icon('heroicon-o-calendar'),
                    TextEntry::make('propertyInboundPaymentMethodLine.name')
                        ->placeholder('-')
                        ->label(__('accounts::filament/resources/partner.infolist.entries.payment-method'))
                        ->icon('heroicon-o-credit-card'),
                ])
                ->columns(2),
        ]);

        $purchaseComponents = Fieldset::make(__('accounts::filament/resources/partner.infolist.entries.purchase'))
            ->schema([
                Group::make()
                    ->schema([
        ...
        $fiscalInformation = Fieldset::make(__('accounts::filament/resources/partner.infolist.entries.fiscal-information'))
            ->schema([
                Group::make()
                    ->schema([
                        TextEntry::make('propertyAccountPosition.name')
                            ->label(__('accounts::filament/resources/partner.infolist.entries.fiscal-position'))
                            ->placeholder('-')
        ...
        $invoicingComponent = Tab::make(__('accounts::filament/resources/partner.infolist.tabs.invoicing.title'))
            ->icon('heroicon-o-receipt-percent')
            ->schema([
                Fieldset::make(__('accounts::filament/resources/partner.infolist.tabs.invoicing.entries.customer-invoices'))
                    ->schema([
                        TextEntry::make('invoice_sending_method')
                            ->label(__('accounts::filament/resources/partner.infolist.tabs.invoicing.entries.invoice-sending-method'))
                            ->placeholder('-')
                            ->icon('heroicon-o-paper-airplane'),
                        TextEntry::make('invoice_edi_format_store')
                            ->label(__('accounts::filament/resources/partner.infolist.tabs.invoicing.entries.invoice-edi-format-store'))
                            ->placeholder('-')
                            ->icon('heroicon-o-document'),
                        Group::make()
                            ->schema([
                                TextEntry::make('peppol_eas')
                                    ->label(__('accounts::filament/resources/partner.infolist.tabs.invoicing.entries.peppol-eas'))
                                    ->placeholder('-')
                                    ->icon('heroicon-o-identification'),
                                TextEntry::make('peppol_endpoint')
                                    ->label(__('accounts::filament/resources/partner.infolist.tabs.invoicing.entries.endpoint'))
                                    ->placeholder('-')
                                    ->icon('heroicon-o-globe-alt'),
                            ])->columns(2),
                    ]),

                Fieldset::make(__('accounts::filament/resources/partner.infolist.tabs.invoicing.entries.automation'))
                    ->schema([
                        TextEntry::make('autopost_bills')
                            ->label(__('accounts::filament/resources/partner.infolist.tabs.invoicing.entries.auto-post-bills'))
                            ->placeholder('-')
                            ->icon('heroicon-o-bolt'),
                        IconEntry::make('ignore_abnormal_invoice_amount')
                            ->boolean()
                            ->placeholder('-')
                            ->label(__('accounts::filament/resources/partner.infolist.tabs.invoicing.entries.ignore-abnormal-invoice-amount')),
                        IconEntry::make('ignore_abnormal_invoice_date')
                            ->boolean()
                            ->placeholder('-')
                            ->label(__('accounts::filament/resources/partner.infolist.tabs.invoicing.entries.ignore-abnormal-invoice-date')),
                    ]),
            ]);
